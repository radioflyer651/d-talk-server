...
export interface IPluginResolver {
    ...
    getPluginInstance(pluginInstance: PluginInstanceReference, attachmentTarget: PluginAttachmentTarget, attachToAttachmentTarget: boolean): Promise<AgentPluginBase | undefined>;

    /** Given a set of PluginInstanceReference objects, returns all plugins for those objects. */
    getPluginInstances(pluginReferences: PluginInstanceReference[], attachmentTarget: PluginAttachmentTarget, attachToAttachmentTarget: boolean): Promise<AgentPluginBase[]>;

    /**
     * Creates a new plugin instance of the specified type, with optional initialization data.
     * Returns the created plugin instance. */
    createPluginInstance(pluginReference: PluginSpecification, attachmentTarget: PluginAttachmentTarget, attachToAttachmentTarget: boolean): Promise<AgentPluginBase>;
    ...
    hydrateAllPlugins(pluginReferences: PluginSpecification[], pluginInstances: PluginInstanceReference[], attachmentTarget: PluginAttachmentTarget, attachToAttachmentTarget: boolean): Promise<{ newPlugins: AgentPluginBase[], existingPlugins: AgentPluginBase[]; pluginsRemoved: boolean; }>;
}