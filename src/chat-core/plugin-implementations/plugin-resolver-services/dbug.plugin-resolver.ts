...
export class DebugPluginResolver implements IPluginTypeResolver<DebugPlugin> {
    ...
    async createNewPlugin(specification: PluginSpecification, attachedTo: PluginAttachmentTarget): Promise<DebugPlugin> {
        const result = new DebugPlugin(specification);
        result.attachedTo = attachedTo;
        return result;
    }

    async hydratePlugin(pluginInstance: PluginInstanceReference, attachedTo: PluginAttachmentTarget): Promise<DebugPlugin> {
