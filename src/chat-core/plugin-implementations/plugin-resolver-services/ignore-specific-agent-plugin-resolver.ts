...
export class IgnoreSpecificAgentPluginResolver implements IPluginTypeResolver<IgnoreSpecificAgentPlugin> {
    ...
    async createNewPlugin(specification: PluginSpecification, attachedTo: PluginAttachmentTarget): Promise<IgnoreSpecificAgentPlugin> {
        const result = new IgnoreSpecificAgentPlugin(specification);
        result.attachedTo = attachedTo;
        return result;
    }

    async hydratePlugin(pluginInstance: PluginInstanceReference, attachedTo: PluginAttachmentTarget): Promise<IgnoreSpecificAgentPlugin> {
