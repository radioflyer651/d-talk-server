...
export class LabelAgentSpeakersPluginResolver implements IPluginTypeResolver<LabelAgentSpeakersPlugin> {
    ...
    async createNewPlugin(spec: PluginSpecification, attachedTo: PluginAttachmentTarget): Promise<LabelAgentSpeakersPlugin> {
        const result = new LabelAgentSpeakersPlugin(spec);
        result.attachedTo = attachedTo;
        return result;
    }

    async hydratePlugin(instance: PluginInstanceReference, attachedTo: PluginAttachmentTarget): Promise<LabelAgentSpeakersPlugin> {
