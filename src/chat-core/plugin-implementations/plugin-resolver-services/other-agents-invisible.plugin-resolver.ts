...
export class OtherAgentsInvisiblePluginResolver implements IPluginTypeResolver<OtherAgentsInvisiblePlugin> {
    ...
    async createNewPlugin(specification: PluginSpecification, attachedTo: PluginAttachmentTarget): Promise<OtherAgentsInvisiblePlugin> {
        const result = new OtherAgentsInvisiblePlugin(specification);
        result.attachedTo = attachedTo;
        return result;
    }

    async hydratePlugin(pluginInstance: PluginInstanceReference, attachedTo: PluginAttachmentTarget): Promise<OtherAgentsInvisiblePlugin> {
